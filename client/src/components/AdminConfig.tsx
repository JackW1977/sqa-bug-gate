...
const AdminConfig: React.FC<Props> = ({ config }) => {
  ...
  useEffect(() => {
    api<{ tokenConfigured: boolean }>('getGleanTokenStatus').then((r) => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const configResult = await api<{ success: boolean; error?: string }>(
        'updateConfig', patch,
      );
      if (!configResult.success) throw new Error((configResult as { error?: string }).error ?? 'Config save failed');

      // Save Glean token if entered
      if (showTokenInput && tokenInput.trim()) {
        const tokenResult = await api<{ success: boolean; error?: string }>(
          'setGleanToken',
          { token: tokenInput.trim(), baseUrl: gleanBaseUrl.trim() },
        );
        if (!tokenResult.success) throw new Error((tokenResult as { error?: string }).error ?? 'Token save failed');
    ...
    <div style={{
      ...
      <div style={sectionCard}>
        ...
        <div style={sectionBody}>
          ...
          {/* API Token */}
          <div style={fieldRow}>
            <FieldLabel hint="Stored in server secrets storage">API Token</FieldLabel>
            <div>
              {tokenConfigured && !showTokenInput ? (
                ...
                <div>
                  ...
                  <p style={{ margin: '4px 0 0', fontSize: '11px', color: '#6B778C' }}>
                    Token is stored in server secrets storage. It is never sent to the browser.
                  </p>
                  {tokenConfigured && (
                    <button
      ...
      <div style={sectionCard}>
        ...
        <div style={sectionBody}>
          <p style={{ margin: '0 0 12px', fontSize: '13px', color: '#5E6C84' }}>
            {config.categories.length} categories configured. Edit via the server configuration.
          </p>
